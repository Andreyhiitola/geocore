...
С 19.07.2026 бэкапы снимаются с **двух** серверов. У каждого региона **свой бакет**:

| Регион | Сервер | Бакет | Compose |
|---|---|---|---|
| RU | FirstVDS | `geocore-backups` | `docker-compose.yml` |
| KZ | hoster.kz `213.109.146.180` | `geocore-backups-kz` | `docker-compose.demo.yml` |

⚠️ **Общий бакет на два региона запрещён.** Ключи `daily/`, `weekly/`, `monthly/`
завязаны только на дату (`db-2026-07-19.sql.gz`), поэтому регионы перезаписывали бы
дампы друг друга. Хуже того, `s3 sync ... --delete` в `moodledata-mirror/` (шаг 4
`scripts/backup.sh`) удалил бы из зеркала всё, чего нет на *текущем* сервере — то
есть один регион снёс бы зеркало другого. Разделение сделано именно бакетами, а не
префиксом внутри одного: префикс защищает лишь пока обе стороны его правильно
проставили, а забытая переменная снова приводит к общему пути.

Настройки `geocore-backups-kz` (создан 19.07.2026): зона `ru-3`, класс
**Стандартный** (холодные классы не подходят — ежедневный `sync` мелких объектов
и ротация через 7/28 дней попадают на плату за запросы и минимальный срок
хранения), приватный, versioning + Object Lock **без default-правила**.

⚠️ **Default-правило retention включать нельзя.** Срок проставляет сам `backup.sh`
пообъектно и только на дампы БД/USN. Бакетный дефолт распространился бы и на
`moodledata-mirror/`, где при неработающем на Selectel lifecycle (см. ниже) старые
версии не истекают — получилась бы растущая гора неудаляемых объектов.

**Доступ KZ:** сервисный пользователь Selectel (IAM → Сервисные пользователи) с
ролью `s3.admin` на проект `My First Project`, S3-ключ `geocore-kz-backup`.
Секреты — в Bitwarden, запись «Selectel — сервисный пользователь S3 (GeoCore KZ)».

**Принятый риск (проверено 19.07.2026):** Selectel выдаёт права S3 на **проект**, а
не на отдельный бакет. Ключ KZ `geocore-kz-backup` с ролью `s3.admin` фактически
листает `geocore-backups` (RU) и, как admin, может туда писать и удалять — это
проверено командой `s3 ls s3://geocore-backups/` с KZ-сервера, не предположение.

Полная изоляция потребовала бы **отдельного проекта** Selectel под KZ (права
проектные, бакет пришлось бы пересоздать там). Решено не делать — RU планируется к
отключению. Что остаётся под угрозой при компрометации KZ-сервера:

- `moodledata-mirror/` RU (~36 ГБ курсов) — **не защищён ничем**, Object Lock на нём
  сознательно не включён;
- дампы БД/USN за последние 35 дней — защищены Object Lock COMPLIANCE (неудаляемы
  даже этим ключом), более старые — нет.

Единственный барьер от случайной беды — правильный `S3_BUCKET` в `.env` на KZ.
Поэтому в `.env.kz.example` стоит явное предупреждение.

**Решение 19.07.2026: изоляцию отложили.** Обоснование: клиентов ещё нет (в дампах
БД нет персональных данных), а курсы продублированы на Google Drive — потеря
`moodledata-mirror/` восстановима.

⏰ **Триггер пересмотра — ДО первого реального клиента**, не после. Причина одна, но
достаточная: с появлением учащихся в дампах БД окажутся ПД (ФИО, email, оценки),
которых нет больше нигде — Google Drive дублирует только курсы, не базу. Объём
переезда роли не играет: 30–40 ГБ внутри ru-3 стоят копейки и качаются быстро.

Как сделать изоляцию: новый проект Selectel → бакет в нём с теми же настройками
(ru-3, Стандартный, приватный, versioning + Object Lock без default-правила) →
сервисный пользователь с `s3.admin` **на новый проект** → новый S3-ключ →
`S3_BUCKET` + `AWS_*` в `/opt/geocore/.env` на KZ. Старый ключ отозвать.
Бакеты между проектами не переносятся — только создать новый и скопировать.

**Резидентность:** бэкапы KZ осознанно остаются в Selectel (РФ) — заказчику важна
зона `.kz` и домен. Если требование окажется юридическим (локализация ПД РК), схему
придётся менять: в дампах Moodle есть ФИО, email и оценки учащихся.
